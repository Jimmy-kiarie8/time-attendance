            ...
            <v-col cols="12" sm="9">
                <v-row>
                    ...
                    <v-col v-for="field in section.fields" :key="field.label" cols="12" sm="4">
                        ...
                        <component :is="getComponentType(field.type)" v-model="email[field.model]"
                            :label="field.label" :items="field.options || []" variant="outlined"
                            v-if="field.type !== 'switch'" density="compact" item-value="value"></component>
                        <v-switch v-model="email[field.model]" :label="field.label" v-if="field.type === 'switch'"
...
onMounted(async () => {
    const response = await axios.get('/data/integrations.json');
